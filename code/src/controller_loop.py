...
class PipelineController_Loop:
    ...
    def run(self):
        if self.config['local_model_config']['test_local_model'] is False and self.config['global_model_config']['test_global_model'] is False:
            print("\nNo model to test. Please set 'test_local_model' or 'test_global_model' to True in the config.\n")
            quit()
        else:
            number_iterations_global, number_iterations_local = 0, 0
            if self.config['local_model_config']['test_local_model'] is True:
                number_iterations_local = self.config["local_model_config"]["num_partitions"]
            if self.config['global_model_config']['test_global_model'] is True:
                number_iterations_global = self.config["global_model_config"]["num_partitions"] 
            number_iterations = max(number_iterations_local, number_iterations_global)
        ...
        for i in range(self.config["min_number_iterarations"], number_iterations + 1): #! iterations count from min to num_partitions inclusive.
            ...
            if self.config["local_model_config"]["test_local_model"] is True:
                self.config["local_model_config"]["num_partitions"] = i #! iterates over num partitions for local model. 
            ...
            if self.config["global_model_config"]["test_global_model"] is True:
                ...
                print(f"\n\nWaiting for {self.config['delay_time']} seconds before next iteration...\n")
